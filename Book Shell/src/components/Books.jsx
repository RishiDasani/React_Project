...
function Books() {
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (row) => (
        <>
          <Link to={`/editbook/${row.id}`} className='bg-gray-300/60 px-5 py-2 font-semibold hover:bg-green-500/40 rounded-lg mx-2'>
            Edit
          </Link>
          {/* <Link to={`/deletebook/${row.id}`} className='bg-green-500/60 px-5 py-2 font-semibold hover:bg-green-500/40 rounded-lg mx-2'>
            Delete
          </Link> */}
    ...
    },
    {
      ...
      renderCell: (row) => (
        <>
          <Link to={`/deletebook/${row.id}`} className='bg-gray-300/60 px-5 py-2 font-semibold hover:bg-green-500/40 rounded-lg mx-2'>
            Delete
          </Link>
          {/* <button onClick={() => deleteBook(row.id)} className='bg-red-500/60 px-5 py-2 font-semibold hover:bg-red-500/40 rounded-lg mx-2'>
            Delete
  ...
  return (
    <><div class="container-2">
      <div className="flex justify-center flex-col items-center p-4">
        <h1 className="text-4xl font-bold text-center p-3">Books List</h1>
        {/* <div className="w-80 h-1 bg-red-400 rounded-full"></div> */}
